...
import type { AskActionState } from "@/server/services/ask";
...
describe("AskPanel", () => {
  it("renders the question form with a read-only note and no answer yet", () => {
    render(<AskPanel action={noopAction} />);
  ...
  it("prefills and submits an initial question from search handoff", async () => {
    ...
    render(<AskPanel action={noopAction} initialQuestion="Who knows Mordecai?" />);
  ...
  it("updates the draft and submits again when the search handoff query changes", async () => {
    ...
    const { rerender } = render(
      <AskPanel action={noopAction} initialQuestion="Who knows Mordecai?" />,
    );
    await waitFor(() => expect(action).toHaveBeenCalledTimes(1));

    rerender(<AskPanel action={noopAction} initialQuestion="Where is Donut?" />);
